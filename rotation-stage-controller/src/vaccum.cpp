...
/**
 * Vacuum Pump Control
 * Drive pump through MOSFET with PWM (and flyback diode across pump if brushed motor).
 */
#define VAC_PWM_PIN   PB0     // pick an available PWM-capable pin on your board
#define VAC_PWM_FREQ  25000   // 25 kHz (above audible)
...
uint16_t vac_hold_duty = (uint16_t)(VAC_PWM_MAX * 0.35f); // 35% (tune)
 ...
 * Vacuum helpers
 */
static inline void vacuum_apply_pwm(uint16_t duty)
{
