...
def main(
    ...
    try:
        ...
        print(f"\nStarting camera comparison with {len(relative_points)} points...")
        print(f"Delay between moves: {delay}s\n")

        # Move through each relative point and capture images
        for idx, (rel_x, rel_y, rel_z) in enumerate(relative_points):
            # Calculate absolute position
            abs_x = x_start + rel_x
            abs_y = y_start + rel_y
            abs_z = z_start + rel_z

            print(f"[{idx + 1}/{len(relative_points)}] Moving to relative ({rel_x}, {rel_y}, {rel_z})")
            print(f"  Absolute position: ({abs_x:.6f}, {abs_y:.6f}, {abs_z:.6f})")

            # Move stage
            oms.set_pose(abs_x, abs_y, abs_z)
            oms.wait_for_stop()
            ...
            if frame is not None:
                # Format filename
                rel_x_str, rel_y_str, rel_z_str = sanitize_filename(rel_x, rel_y, rel_z)
                final_x_str, final_y_str, final_z_str = sanitize_filename(final_x, final_y, final_z)

                filename = (
                    f"{idx:04d}_{rel_x_str}_{rel_y_str}_{rel_z_str}_"
                    f"{final_x_str}_{final_y_str}_{final_z_str}.png"
                )
